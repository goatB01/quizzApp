...
function showQuetions(index) {
  ...
  let que_tag =
    ...
    "</span>";
  var arrayLength = questions[index].options.length;
  let option_tag = "";
  ...
  for (let i = 0; i < option.length; i++) {
    option[i].addEventListener("click", function () {
      ...
      if (correcAns.includes(userAns)) {
        ...
        correctAnswers++;
        if (correctAnswers === questions[que_count].answer.length) {
          clearInterval(counter); //clear counter
          clearInterval(counterLine); //clear counterLine
          optionSelected(true);
        }
...
function optionSelected(isCorrectAnswer) {
  ...
  if (isCorrectAnswer) {
    ...
    for (i = 0; i < allOptions; i++) {
      ...
      if (correcAns.includes(option_list.children[i].textContent)) {
...
function startTimerLine(time) {
  ...
  function timer() {
    ...
    if (time > 1050) {
      //if time value is greater than 1050
      clearInterval(counterLine); //clear counterLine
    }
  }
}
